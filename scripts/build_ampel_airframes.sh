...
main() {
    ...
    for i in $(seq 1 8); do
        create_ampel_system $sys_num "HWB" "Hybrid Wing Body" "AMPEL-C" "Examples: Boeing X-48, Airbus A-XXX"
        sys_num=$((sys_num + 1))
    done
    
    # FLW: Flying Wing (10 systems)
    for i in $(seq 1 10); do
        create_ampel_system $sys_num "FLW" "Flying Wing" "AMPEL-C" "Examples: B-2 Spirit, Northrop YB-49"
        sys_num=$((sys_num + 1))
    done
    
    # TBW: Truss-Braced Wing (8 systems)
    for i in $(seq 1 8); do
        create_ampel_system $sys_num "TBW" "Truss-Braced Wing" "AMPEL-C" "Examples: NASA TTBW, Boeing SUGAR Volt"
        sys_num=$((sys_num + 1))
    done
    
    # BOX: Box Wing (6 systems)
    for i in $(seq 1 6); do
        create_ampel_system $sys_num "BOX" "Box Wing" "AMPEL-C" "Examples: Lockheed Box Plane"
        sys_num=$((sys_num + 1))
    done
    
    # JOW: Joined Wing (5 systems)
    for i in $(seq 1 5); do
        create_ampel_system $sys_num "JOW" "Joined Wing" "AMPEL-C" "Examples: Wolkovitch Joined Wing"
        sys_num=$((sys_num + 1))
    done
    
    # TDW: Tandem Wing (6 systems)
    for i in $(seq 1 6); do
        create_ampel_system $sys_num "TDW" "Tandem Wing" "AMPEL-C" "Examples: Quickie, Rutan Quickie"
        sys_num=$((sys_num + 1))
    done
    
    # CAN: Canard (5 systems)
    for i in $(seq 1 5); do
        create_ampel_system $sys_num "CAN" "Canard" "AMPEL-C" "Examples: Rutan VariEze, Piaggio P.180"
        sys_num=$((sys_num + 1))
    done
    
    # TSF: Three-Surface (5 systems)
    for i in $(seq 1 5); do
        create_ampel_system $sys_num "TSF" "Three-Surface" "AMPEL-C" "Examples: Piaggio P.180 Avanti"
        sys_num=$((sys_num + 1))
    done
    ...
    for i in $(seq 1 8); do
        create_ampel_system $sys_num "DEL" "Delta Wing" "AMPEL-D" "Examples: Concorde, Mirage 2000"
        sys_num=$((sys_num + 1))
    done
    
    # VGW: Variable Geometry (6 systems)
    for i in $(seq 1 6); do
        create_ampel_system $sys_num "VGW" "Variable Geometry" "AMPEL-D" "Examples: F-14 Tomcat, Tornado"
        sys_num=$((sys_num + 1))
    done
    
    # FSW: Forward-Swept Wing (4 systems)
    for i in $(seq 1 4); do
        create_ampel_system $sys_num "FSW" "Forward-Swept Wing" "AMPEL-D" "Examples: X-29, Sukhoi Su-47"
        sys_num=$((sys_num + 1))
    done
    ...
    for i in $(seq 1 4); do
        create_ampel_system $sys_num "CSW" "Compound Swept" "AMPEL-D" "Examples: Advanced swept configurations"
        sys_num=$((sys_num + 1))
    done
    ...
    for i in $(seq 1 5); do
        create_ampel_system $sys_num "BIP" "Biplane" "AMPEL-M" "Examples: Boeing Stearman, Pitts Special"
        sys_num=$((sys_num + 1))
    done
    
    # TRP: Triplane (5 systems)
    for i in $(seq 1 5); do
        create_ampel_system $sys_num "TRP" "Triplane" "AMPEL-M" "Examples: Fokker Dr.I"
        sys_num=$((sys_num + 1))
    done
    
    # MUP: Multi-plane (5 systems)
    for i in $(seq 1 5); do
        create_ampel_system $sys_num "MUP" "Multi-plane" "AMPEL-M" "Examples: Historical multi-wing aircraft"
        sys_num=$((sys_num + 1))
    done
    
    # STP: Staggered (5 systems)
    for i in $(seq 1 5); do
        create_ampel_system $sys_num "STP" "Staggered" "AMPEL-M" "Examples: Beechcraft Staggerwing"
        sys_num=$((sys_num + 1))
    done
    
    # CHW: Channel Wing (5 systems)
    for i in $(seq 1 5); do
        create_ampel_system $sys_num "CHW" "Channel Wing" "AMPEL-M" "Examples: Custer Channel Wing"
        sys_num=$((sys_num + 1))
    done
    ...
    for i in $(seq 1 4); do
        create_ampel_system $sys_num "RNG" "Ring Wing" "AMPEL-N" "Examples: Burnelli Lifting Body"
        sys_num=$((sys_num + 1))
    done
    
    # ANN: Annular (4 systems)
    for i in $(seq 1 4); do
        create_ampel_system $sys_num "ANN" "Annular" "AMPEL-N" "Examples: Circular wing concepts"
        sys_num=$((sys_num + 1))
    done
    
    # LFB: Lifting Body (4 systems)
    for i in $(seq 1 4); do
        create_ampel_system $sys_num "LFB" "Lifting Body" "AMPEL-N" "Examples: X-24, M2-F3"
        sys_num=$((sys_num + 1))
    done
    
    # WIG: Wing-in-Ground (4 systems)
    for i in $(seq 1 4); do
        create_ampel_system $sys_num "WIG" "Wing-in-Ground" "AMPEL-N" "Examples: Ekranoplan, Airfish 8"
        sys_num=$((sys_num + 1))
    done
    
    # DUC: Ducted (4 systems)
    for i in $(seq 1 4); do
        create_ampel_system $sys_num "DUC" "Ducted" "AMPEL-N" "Examples: Ducted fan aircraft"
        sys_num=$((sys_num + 1))
    done
    ...
    for i in $(seq 1 3); do
        create_ampel_system $sys_num "DPW" "Distributed Propulsion" "AMPEL-P" "Examples: NASA X-57 Maxwell"
        sys_num=$((sys_num + 1))
    done
    
    # BLI: Boundary Layer Ingestion (3 systems)
    for i in $(seq 1 3); do
        create_ampel_system $sys_num "BLI" "Boundary Layer Ingestion" "AMPEL-P" "Examples: MIT D8 concept"
        sys_num=$((sys_num + 1))
    done
    ...
    for i in $(seq 1 3); do
        create_ampel_system $sys_num "FAN" "Fan Wing" "AMPEL-P" "Examples: Fanwing aircraft"
        sys_num=$((sys_num + 1))
    done
    
    # CYC: Cycloidal (3 systems)
    for i in $(seq 1 3); do
        create_ampel_system $sys_num "CYC" "Cycloidal" "AMPEL-P" "Examples: Cyclocopter concepts"
        sys_num=$((sys_num + 1))
    done
    ...
    for i in $(seq 1 2); do
        create_ampel_system $sys_num "MOR" "Morphing Wing" "AMPEL-A" "Examples: DARPA Morphing Aircraft"
        sys_num=$((sys_num + 1))
    done
    
    # ADP: Adaptive (2 systems)
    for i in $(seq 1 2); do
        create_ampel_system $sys_num "ADP" "Adaptive" "AMPEL-A" "Examples: Variable camber wings"
        sys_num=$((sys_num + 1))
    done
    
    # SMT: Smart Materials (2 systems)
    for i in $(seq 1 2); do
        create_ampel_system $sys_num "SMT" "Smart Materials" "AMPEL-A" "Examples: Shape memory alloy wings"
        sys_num=$((sys_num + 1))
    done
    
    # BIO: Biomimetic (2 systems)
    for i in $(seq 1 2); do
        create_ampel_system $sys_num "BIO" "Biomimetic" "AMPEL-A" "Examples: Bird-inspired designs"
        sys_num=$((sys_num + 1))
    done
    
    # FLD: Folding (2 systems)
    for i in $(seq 1 2); do
        create_ampel_system $sys_num "FLD" "Folding" "AMPEL-A" "Examples: Folding wing aircraft"
        sys_num=$((sys_num + 1))
    done
    ...
    create_ampel_system $sys_num "TLT" "Tiltrotor" "AMPEL-V" "Examples: V-22 Osprey, Bell XV-15"
    sys_num=$((sys_num + 1))
    
    # VTL: VTOL (1 system)
    create_ampel_system $sys_num "VTL" "VTOL" "AMPEL-V" "Examples: Harrier Jump Jet, F-35B"
    sys_num=$((sys_num + 1))
    
    # STO: STOL (1 system)
    create_ampel_system $sys_num "STO" "STOL" "AMPEL-V" "Examples: DHC-6 Twin Otter, STOL aircraft"
    sys_num=$((sys_num + 1))
    
    # CMP: Compound (1 system)
    create_ampel_system $sys_num "CMP" "Compound" "AMPEL-V" "Examples: Sikorsky X2, S-97 Raider"
    sys_num=$((sys_num + 1))
    
    # QDR: Quadrotor (1 system)
    create_ampel_system $sys_num "QDR" "Quadrotor" "AMPEL-V" "Examples: Large manned quadcopters"
